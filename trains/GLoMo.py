...
def train_epoch(model: nn.Module, train_dataloader: DataLoader, epoch=None):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        # b_s = combined_labels.size(0) // 2
        # y_unmix_pre, y_mix_pre = outputs[0:b_s], outputs[b_s:]
        # y_unmix_true, y_mix_true = combined_labels[0:b_s], combined_labels[b_s:]

        loss_fct = L1Loss()
        # mix_cls_loss = nn.MSELoss(reduction='none')

        # mix_cls_loss = mix_cls_loss(y_mix_pre, y_mix_true)
        # loss_all = loss_fct(logits.view(-1), combined_labels.view(-1)) + moe_losses + 0.7 * mix_cls_loss.mean() + 0.5 * kl_loss
...
def train(
    ...
    test_accuracies = []
    f1_scores = []
    all_metrics = []  # 存储所有epoch的指标
    best_avg_metric = -1  # 初始化为最小值
    best_epoch = -1
    best_metrics = {}  # 存储最佳epoch的所有指标
    best_loss = 10
    for epoch_i in range(int(args.n_epochs)):
        ...
        avg_metric = (test_acc7 + test_acc5 + test_acc2_non_zero + test_f_score_non_zero + test_acc2 + test_f_score) / 6
        ...
        print(
            "current mae:{:.4f}, current corr:{:.4f}, acc7:{:.4f}, acc5:{:.4f}, acc2_non_zero:{:.4f}, f_score_non_zero:{:.4f}, acc2:{:.4f}, f_score:{:.4f}, Avg:{:.4f}".format(
                test_mae, test_corr, test_acc7, test_acc5, test_acc2_non_zero, test_f_score_non_zero, test_acc2,
                test_f_score, avg_metric
            )
        )
    ...
    print(f"Acc2非零: {best_metrics['test_acc2_non_zero']:.4f}")
    print(f"F-score非零: {best_metrics['test_f_score_non_zero']:.4f}")
    print(f"Acc2: {best_metrics['test_acc2']:.4f}")
    print(f"F-score: {best_metrics['test_f_score']:.4f}")
    print(f"Acc7: {best_metrics['test_acc7']:.4f}")
    print(f"Acc5: {best_metrics['test_acc5']:.4f}")
    print(f"Avg指标: {best_metrics['avg_metric']:.4f}")

    print(f"测试MAE: {best_metrics['test_mae']:.4f}")
