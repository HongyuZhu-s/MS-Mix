...
class ALMT():
    def __init__(self, args):
        ...
        self.criterion1 = nn.MSELoss(reduction='none') if args.train_mode == 'regression' else nn.CrossEntropyLoss(
            reduction='none')
        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for index, batch_data in enumerate(td):
                    ...
                    optimizer.zero_grad()
                    # text, audio, vision = mask_features(text), mask_features(audio), mask_features(vision)

                    _, o, shuffled_labels1, shuffled_labels2, lam_labels, combined_labels, kl_loss = model(text,
                                                                                                           audio, vision, labels)

                    b_s = combined_labels.size(0) // 2

                    y_unmix_pre, y_mix_pre = o[0:b_s], o[b_s:]
                    y_unmix_true, y_mix_true = combined_labels[0:b_s], combined_labels[b_s:]

                    cls_loss1, cls_loss2 = \
                        self.criterion1(y_mix_pre, shuffled_labels1), self.criterion1(y_mix_pre, shuffled_labels2)
                    mix_cls_loss = cls_loss1 * lam_labels + cls_loss2 * (1 - lam_labels)

                    cls_loss3 = self.criterion(y_unmix_pre, y_unmix_true)

                    loss = mix_cls_loss.mean() * 0.7 + kl_loss * 0.5 + cls_loss3

                    loss.backward()
                    optimizer.step()
                    logits = o.detach().cpu()
                    labels = combined_labels.detach().cpu()
    ...
    def do_test(self, model, dataloader, mode="VAL"):
        ...
        with torch.no_grad():
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    outputs, _, _, _, _, _, _ = model(text, audio, vision, labels)
        ...
        keys_to_avg = list(results.keys())[:6]
        avg = sum(results[key] for key in keys_to_avg) / len(keys_to_avg)

        results["Loss"], results["Avg."] = round(eval_loss, 4), round(avg, 4)
