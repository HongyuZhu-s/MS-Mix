...
class MULT():
    def __init__(self, args):
        ...
        # 创建保存数据的目录
        self.data_save_dir = f"./saved_data/{args.dataset_name}"
        os.makedirs(self.data_save_dir, exist_ok=True)

    def do_train(self, model, dataloader, return_epoch_results=False):
        optimizer = optim.Adam(model.parameters(), lr=self.args.learning_rate)
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=self.args.patience)
        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # forward
                    res = model(text, audio, vision, labels)
                    outputs = res['M']
