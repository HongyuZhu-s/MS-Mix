...
class MISA():
    def __init__(self, args):
        self.args = args
        self.criterion = nn.MSELoss(reduction='none') if args.train_mode == 'regression' else nn.CrossEntropyLoss(reduction='none')
        self.criterion1 = nn.MSELoss() if args.train_mode == 'regression' else nn.CrossEntropyLoss()
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        if 'cuda' in str(self.args.device):
            _ = torch.tensor([1.0], device=self.args.device) @ torch.tensor([1.0], device=self.args.device)
        ...
        while True: 
            ...
            with (tqdm(dataloader['train']) as td):
                for batch_data in td:
                    ...
                    # forward
                    outputs = model(text, audio, vision, labels)['M']
    ...
    def do_test(self, model, dataloader, mode="VAL", return_sample_results=False):
        ...
        with torch.no_grad():
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    outputs = model(text, audio, vision, labels)
                    ...
                    loss = self.criterion1(outputs['M'], labels)
                    eval_loss += loss.item()
        ...
        keys_to_avg = list(eval_results.keys())[:6]
        avg = sum(eval_results[key] for key in keys_to_avg) / len(keys_to_avg)

        eval_results["Loss"], eval_results["Avg."] = round(eval_loss, 4), round(avg, 4)
