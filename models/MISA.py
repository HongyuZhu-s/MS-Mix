...
from mix_method import vanilla_mixup, ME_mixup, MultiMix, PowMix, discretize_labels
from MS_Mix_code.ME_MIX import ME_test, IntensityPredictor
...
class MISA(nn.Module):
    ...
    def forward(self, text, audio, video, y):
        for modality in self.intensity_predictors:
            self.intensity_predictors[modality] = self.intensity_predictors[modality].to(text.device)
        output = self.alignment(text, audio, video)
        tmp = {
            "M": output
        }
        # o, shuffled_labels1, shuffled_labels2, lam_expanded_labels, combined_labels, kl_loss = \
        #     self.mix_alignment(text, audio, video, y)
        return tmp
