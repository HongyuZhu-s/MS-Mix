...
# from ME_MIX import ME_test, IntensityPredictor
...
class GLoMo(BertPreTrainedModel):
    def __init__(self, config, args = None):
        ...
        input_dims = {'audio': 96,  # 音频特征维度   # mosei 17  mosi  17     17
                      'text': 96,  # 文本特征维度    #      65       65    33
                      'vision': 96}  # 视觉特征维度  #       129       129     65
    ...
    def forward(
        ...
        # if coarsed_a.size(0) != 1:
        #     coarsed_a, coarsed_t, coarsed_v, _, _, _, combined_labels, kl_loss = ME_test(coarsed_a, coarsed_t, coarsed_v, label_ids, intensity_predictors=self.intensity_predictors)
        #     fine_as, fine_ts, fine_vs, _, _, _, _, _ = ME_test(fine_as, fine_ts, fine_vs, label_ids, intensity_predictors=self.intensity_predictors)
        # else:
        #     combined_labels = label_ids
        #     kl_loss = coarsed_a.new_tensor(0.0)
        ...
        # return all_losses, logits, outputs, combined_labels, kl_loss
