...
from MS_MIX import MS_Mix_threeD
...
class ALMT(nn.Module):
    def __init__(self, args):
        ...
        self.l_encoder_hidden_d = self.h_hyper_d = args.dst_feature_dims   # 128
        self.l_encoder_heads = args.l_encoder_heads    # 8
        self.h_hyper_length = args.dst_embedding_length   # 8
        
        self.h_hyper = nn.Parameter(torch.ones(1, self.h_hyper_length, self.h_hyper_d)) # [1, 8, 128]
        ...
        self.l_encoder = Transformer(
            num_frames=self.dst_embedding_length_l,  # 8
            save_hidden=True, 
            token_len=None, 
            dim=self.dst_embedding_d_l, 
            depth=self.AHL_depth-1, 
            heads=self.l_encoder_heads, 
            mlp_dim= self.l_encoder_hidden_d)      # 128
        ...
        input_dims = {'audio': 128,  # 音频特征维度   # mosei 17  mosi  17     17
                      'text': 128,  # 文本特征维度    #      65       65    33
                      'vision': 128}  # 视觉特征维度  #       129       129     65

        self.intensity_predictors = nn.ModuleDict({
            'audio': IntensityPredictor(input_dims['audio']),
            'text': IntensityPredictor(input_dims['text']),
            'vision': IntensityPredictor(input_dims['vision'])
        })


    def forward(self, text, audio, video, y):
        b = video.size(0)
        h_hyper = repeat(self.h_hyper, '1 n d -> b n d', b = b)
        h_hyper1 = repeat(self.h_hyper, '1 n d -> b n d', b = 2 * b)
        ...
        feat = self.fusion_layer(h_hyper, h_l_list[-1])
        feat = feat[:, 0]
        output = self.regression_head(feat)

        m_audio_h, m_text_h, m_video_h, shuffled_labels1, shuffled_labels2, \
            lam_expanded_labels, combined_labels, kl_loss = MS_Mix(h_a, h_l, h_v, y,
                                                           intensity_predictors=self.intensity_predictors)

        mh_l_list = self.l_encoder(m_text_h)
        mh_hyper = self.h_hyper_layer(mh_l_list, m_audio_h, m_video_h, h_hyper1)
        mfeat = self.fusion_layer(mh_hyper, mh_l_list[-1])[:, 0]
        o = self.regression_head(mfeat)

        return output, o, shuffled_labels1, shuffled_labels2, lam_expanded_labels, combined_labels, kl_loss
